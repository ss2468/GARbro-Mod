...
namespace GARbro {
    ...
    class ConsoleBrowser {
        ...
        static void Main(string[] args) {
            ...
            var browser = new ConsoleBrowser();
            browser.DeserializeGameData();
            string path = @"C:\Users\Administrator\Desktop\mes.arc";
            // string path = @"C:\Users\Administrator\Desktop\sall.snl";
            foreach (var file in VFS.GetFiles(path)) {
                string m_arc_name_1 = file.Name;
                try {
                    VFS.ChDir(m_arc_name_1);
                }
                catch (Exception) {
                    Console.Error.WriteLine("{0}: unknown format", m_arc_name_1);
                    continue;
                }
